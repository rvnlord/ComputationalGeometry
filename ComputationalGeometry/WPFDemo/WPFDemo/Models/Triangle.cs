...
namespace WPFDemo.Models
{
    public class Triangle
    {
        ...
        public bool ContainsInCircumcircle(Point p)
        {
            ...
            var p2 = Vertex2;
            var p3 = Vertex3;
            if (Math.Abs(p1.Y - p2.Y) < EPSILON && Math.Abs(p2.Y - p3.Y) < EPSILON) // < double.Epsilon
                return false;
            ...
            if (Math.Abs(p2.Y - p1.Y) < EPSILON) // < double.Epsilon
            {
            ...
            else if (p3.Y - p2.Y < EPSILON) // < double.Epsilon
            {
