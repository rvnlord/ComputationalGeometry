...
namespace WPFDemo.Models
{
    public struct Line
    {
        ...
        public bool Contains(Point p)
        {
            ...
            return Math.Abs(Slope * p.X + Constant - p.Y) < EPSILON;
        }
        ...
        public override bool Equals(object o)
        {
            ...
            if (o == null || GetType() != o.GetType())
                return false;
            var that = (Line)o;
            return Math.Abs(Slope - that.Slope) < EPSILON && Math.Abs(Constant - that.Constant) < EPSILON;
        }
        ...
        {
            if (Math.Abs(Slope - that.Slope) < EPSILON)
                return null;
        ...
        {
            return Math.Abs(Slope - that.Slope) > EPSILON;
        }
        ...
        public bool isPerpendicularTo(Line that)
        {
            ...
            if (that.IsVertical())
                return IsHorizontal();
            return Math.Abs(Slope * that.Slope - (-1)) < EPSILON;
        }
        ...
        public override string ToString()
        {
            ...
            if (Math.Abs(Slope - (-1)) < EPSILON) // Slope
                sb.Append("-x");
            else if (Math.Abs(Slope - 1) < EPSILON)
                sb.Append("x");
            else if (Math.Abs(Slope) > EPSILON)
                sb.Append(Slope).Append("*x");

            if (Math.Abs(Constant) > EPSILON) // Constant
            {
