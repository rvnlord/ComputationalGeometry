...
namespace WPFDemo.Models
{
    public static class GeometryCalculations
    {
        ...
        {
            return Math.Abs(PointOrientationTest(point, segment)) < EPSILON; // == 0
        }

        public static bool LineRactangularBoundsIntersection(LineSegment segment1, LineSegment segment2)
        {
        ...
        {
            if (!LineRactangularBoundsIntersection(segment1, segment2))
                return false;
        ...
        public static bool PointIsLessOrEqualTo(Point p1, Point p2, Point o)
        {
            ...
            return pos > 0 || (pos.EqualsStrict(0) && op1.Length() <= op2.Length());
        }
        ...
        public static List<Point> ConvexHullGraham(List<Point> points)
        {
            ...
            var pn = AngularSort(points.Where(p => p != p0).ToList(), p0)
                ...
                .Select(g => g.MaxBy(p => D(p0, p))).ToList();
        ...
        public static LineSegment FarthestPoints(List<Point> points)
        {
            ...
            return antipodalPairs.MaxBy(pair => Math.Abs(pair.Length()));
        }
        ...
        public static Dictionary<Point, List<LineSegment>> IntersectionsBentleyOttmann(List<LineSegment> segments, out LineSegment[] sweepLines)
        {
            ...
            var minY = endpoints.MinBy(p => p.Y).Y;
            var maxY = endpoints.MaxBy(p => p.Y).Y;
        ...
        {
            if (points.Count < 3)
                throw new ArgumentException("Nie można triangulować mniej niż trzech punktów");
        ...
        public static bool IsInsideConvexPolygon(PointFr point, PolygonFr polygon)
        {
            ...
            if (vertices.Count < 3)
                throw new ArgumentException("Wielokąt nie może mieć poniżej trzech wierzchołków");
            if (ConvexHullJarvis(vertices).Count != vertices.Count) // Powinno wystarczyć Count
                throw new ArgumentException("Wielokąt musi być wypukły");
            ...
            if (polyCenter == null)
                throw new Exception("Nie można obliczyć środka wielokąta");
            ...
            while (edges.Count > 1)
            {
                ...
                if (point.IsRightOf(testSegment))
                    ...
                    throw new Exception("Program źle wykonuje obliczenia, to nie powinno się zdarzyć");
            }
